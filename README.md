...
`ryu64-cleanroom` is a clean-room, MIT-licensed portable C11 library for IEEE-754 binary64 (`double`) decimal formatting and parsing, designed with WebAssembly MVP and no-heap integration in mind.
...
- Design choice:
  - `ryu64_to_shortest` preserves sign (including signed NaN) as an information-preserving conversion.
...
- Full parser (`ryu64_from_decimal_full`) with `RYU64_ENABLE_PARSE_BIGINT`:
  ...
  - Eisel-Lemire fast path for non-truncated significands (`<=19` digits) in exponent range `[-342, 308]`:
    - single 64x128-bit multiply using a precomputed 651-entry table of 128-bit approximations of `5^q` (generated by `tools/gen_pow5_128.c`)
  ...
  - fixed-width integer fast path for non-truncated mantissas (`<=19` significant digits) with decimal exponent in `[-38, 38]`
...
- `RYU_BIGINT_MAX_LIMBS` defaults: `512` native, `256` wasm32.
- Minimum supported value is `96` (compile-time guard).
- Stack budget (measured with `-O2 -fstack-usage`):
  - wasm32 (limbs=256):
...
- Override example:
  - `make CFLAGS_BASE='-std=c11 -O2 -Wall -Wextra -Wpedantic -Iinclude -DRYU_BIGINT_MAX_LIMBS=768u'`
...
- GitHub Actions workflow (`.github/workflows/ci.yml`) runs `make test`, `make nolibc-check-speed`, and `make nolibc-check-size` on both macOS and Linux.
...
- `test/oracle_stdio.c` is a dedicated libc-using test oracle that compares:
  ...
  - shortest via `scanf` roundtrip checks
...
- The parser uses the Eisel-Lemire algorithm (Lemire 2021, Mushtak-Lemire 2023) for the common case (`<=19` significant digits), providing performance competitive with or faster than libc `strtod`. A bigint-backed fallback handles longer inputs.
- Differential oracles and fuzzing are used to validate correctness despite the formatting performance-oriented compromise.
